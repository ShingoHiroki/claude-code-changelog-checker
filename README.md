...
- Slack Webhook へ **Block Kit** でコンパクトに通知（header / 件数サマリー / サイトへのリンクのみ。詳細は公開サイトで閲覧）。未設定時はスキップ。失敗時はプレーンテキストにフォールバック
- Discord にはカテゴリ別の変更内容を**全件**掲載（1メッセージ上限に応じて field 分割・複数 Embed）
